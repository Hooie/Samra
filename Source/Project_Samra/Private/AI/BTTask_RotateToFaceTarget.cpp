...
{
	if (!IsValid(QueryPawn) || !IsValid(TargetActor))
		return false;

	const FVector PawnLoc = QueryPawn->GetActorLocation();
	const FVector TargetLoc = TargetActor->GetActorLocation();

	const FRotator CurrentRot = QueryPawn->GetActorRotation();
	const FRotator DesiredRot = UKismetMathLibrary::FindLookAtRotation(PawnLoc, TargetLoc);

	const float DeltaYaw = FMath::Abs(FMath::FindDeltaAngleDegrees(CurrentRot.Yaw, DesiredRot.Yaw));
	return (DeltaYaw <= AnglePrecision);


	//const FVector OwnerForward = QueryPawn->GetActorForwardVector();
	//const FVector OwnerToTargetNormalized = (TargetActor->GetActorLocation() - (QueryPawn->GetActorLocation() + QueryPawn->GetActorForwardVector() * -50.f )).GetSafeNormal();
	//
